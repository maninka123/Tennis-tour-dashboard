...
const TOUR_LABEL = APP_CONFIG.tourName || 'ATP';
const HERO_SUBTITLE_BASE = 'Player Explorer, Tournament Explorer, and Records Book powered by your local CSV archive';
...
function renderHeroSubtitle() {
  ...
  const maxYear = Number.isFinite(coverage?.maxYear) ? coverage.maxYear : APP_CONFIG.liveYear;
    ...
    <div class="player-hero-head">
      ...
      <div>
        ...
        <div class="player-meta-line">
          ...
          ${player.profileUrl ? `<span>•</span><a href="${escapeHtml(player.profileUrl)}" target="_blank" rel="noopener noreferrer">${escapeHtml(TOUR_LABEL)} Profile</a>` : ''}
        </div>
      </div>
    </div>
  `;
...
function renderPlayerRanking() {
  ...
  if (window.Plotly && dom.rankingChart) {
    ...
    const layout = {
      ...
      yaxis: {
        title: { text: `${TOUR_LABEL} Ranking (Lower is better)`, standoff: 12 },
        visible: showRank,
        automargin: true,
        autorange: 'reversed',
...
function buildRecordMetrics(row, holder) {
  ...
  if (label.includes('1000 titles')) {
    metrics.push(`Masters 1000 titles: ${formatNumber(holder?.mastersTitles || 0)}`);
    metrics.push(`Total titles: ${formatNumber(holder?.titles || 0)}`);
    metrics.push(`Finals reached: ${formatNumber(holder?.finals || 0)}`);
    return metrics;
  }
  ...
  if (label.includes('500 titles')) {
    metrics.push(`ATP 500 titles: ${formatNumber(holder?.atp500Titles || 0)}`);
    metrics.push(`Total titles: ${formatNumber(holder?.titles || 0)}`);
    metrics.push(`Finals reached: ${formatNumber(holder?.finals || 0)}`);
    return metrics;
  }
  ...
  if (label.includes('250 titles')) {
    metrics.push(`ATP 250 titles: ${formatNumber(holder?.atp250Titles || 0)}`);
    metrics.push(`Total titles: ${formatNumber(holder?.titles || 0)}`);
    metrics.push(`Finals reached: ${formatNumber(holder?.finals || 0)}`);
    return metrics;
  }
